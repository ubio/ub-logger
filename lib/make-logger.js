...
function makeLoggerForSeverity(severity, stream, isMetric = false) {
...
function makeLogger(streams = [], logSeverity) {
    ...
    for (const { severity, stream, isMetric } of streams) {
        const inactive = logSeverity === 'mute' || severities.indexOf(severity) < logSeverityIndex;

        logger[severity] = inactive ? noop : makeLoggerForSeverity(severity, stream, isMetric);
    }
